...
export default function PortalAccess() {
  ...
  const handleShopAdminLogin = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        // UPDATED: Redirect to shop dashboard
      ...
      {activeTab === 'shopadmin' && (
        <form onSubmit={handleShopAdminLogin} style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '10px' }}>
            <p style={{ margin: 0 }}>
              New Shop? <Link href="/signup" style={{ color: '#4CAF50', fontWeight: 'bold' }}>Register</Link>
            </p>
            {/* --- NEWLY ADDED LINK --- */}
            <p style={{ margin: 0 }}>
              <Link href="/forgot-password" style={{ color: '#4CAF50', fontWeight: 'bold' }}>Forgot Password?</Link>
            </p>
          </div>
        </form>
      )}
