...
//   let shopId;
//   let isUnique = false;
//   // Initialize counter based on the current number of shops + 1
//   let counter = await Shop.countDocuments() + 1; 
  
//   while (!isUnique) {
//     // Generate SHOP001, SHOP002, etc., using the incrementing counter
//     shopId = `SHOP${String(counter).padStart(3, '0')}`;
    
//     const exists = await Shop.findOne({ shopId });
//     if (!exists) {
//       isUnique = true;
//     } else {
//       // If the generated ID already exists, increment the counter and check the next ID.
//       counter++; 
//     }
//   }
...
//     requestRecord.shopId = shopId;
//     requestRecord.shopCode = shopCode;
//     // THIS LINE WILL NOW SUCCEED because OTP and otpExpires are no longer required.
//     await requestRecord.save(); 
//     console.log("Request updated to approved");
  ...
  try {
    ...
    if (existingShop) {
      ...
      shop = await Shop.create({
        ...
        city: requestRecord.city,
        state: requestRecord.state,
        status: 'pending', // Shop created but pending Super Admin approval
      });
      console.log("Shop created successfully with status: pending");
    }
    ...
    await ShopAdmin.create({
      ...
      password: requestRecord.password,
      shopId: shop._id,
      status: 'pending', // Admin created but pending Super Admin approval
    });
